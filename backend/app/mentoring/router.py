...
from ..models.models import Mentor, Student, Allocation, Meeting
from .schemas import (
...
@mentors_router.post("/", response_model=MentorResponse, status_code=status.HTTP_201_CREATED)
...
@mentors_router.put("/{mentor_id}", response_model=MentorResponse)
...
@mentors_router.delete("/{mentor_id}")
...
@allocations_router.get("/", response_model=List[dict])
...
@allocations_router.get("/details", response_model=List[AllocationResponse])
...
@allocations_router.get("/unallocated", response_model=List[dict])
...
@meetings_router.post("/", response_model=dict, status_code=status.HTTP_201_CREATED)
...
@meetings_router.put("/{meeting_id}", response_model=dict)
def update_existing_meeting(meeting_id: str, payload: MeetingUpdate, db: Session = Depends(get_db)):
    ...
    db_m.updated_at = datetime.utcnow().isoformat()
    db.commit()
    return {"success": True}
...
@meetings_router.get("/mentor/{mentor_id}", response_model=List[MeetingResponse])
...
@dashboard_router.get("/stats", response_model=AllocationStats)
def get_dashboard_stats_endpoint(db: Session = Depends(get_db)):
    return get_allocation_stats(db)

@dashboard_router.get("/workload", response_model=List[dict])
...
@dashboard_router.get("/risk-summary")
...
@dashboard_router.get("/upcoming-meetings")
def get_dashboard_upcoming_meetings(db: Session = Depends(get_db)):
    ...
    meetings_sorted = sorted(
        meetings,
        key=lambda m: (m.date or "", m.time or "")
    )[:5]
