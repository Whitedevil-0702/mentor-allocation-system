...
export default function Students() {
  ...
  const reload = useCallback(async () => {
    try {
      ...
      setStudents(s);
      const map = {};
      details.forEach((a) => { map[a.student_id] = a.mentor_name; });
      setAllocationMap(map);
      setAllocatedCount(s.filter((st) => map[st.student_id]).length);
    } catch (err) {
  ...
  const columns = [
    ...
    { key: 'division', label: 'Division', sortable: true },
    {
      ...
      render: (val) => <span className={val === '—' ? 'text-muted' : 'text-success'}>{val}</span>,
    },
  ];
  ...
  if (loading) {
    return <div className="flex-center" style={{ height: '50vh' }}><p className="text-muted">Loading students...</p></div>;
  }
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="section">
        ...
        <DataTable
          columns={columns} data={tableData} searchable searchPlaceholder="Search students..."
      ...
      <Modal isOpen={showClearModal} onClose={() => setShowClearModal(false)} title="Clear All Students?" size="sm"
        ...
        <p>This will remove all {students.length} student records. Existing allocations will still be preserved.</p>
      </Modal>
    </div>
  );
}